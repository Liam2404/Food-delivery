...
const restaurantLogin = async (req, res) => {
    ...
    try {
        ...
        req.db.query(sql, [email_address], async (err, results) => {
            ...
            req.session.restaurant = {
                id: results[0].id,
                name: results[0].name, // Nom du restaurant (remplacez 'username' par 'name' si nécessaire)
    ...
    const addMeal = async (req, res) => {
        if (!req.session.restaurant) {  // Vérifiez la session du restaurant
            console.log('Session restaurant non trouvée:', req.session);
            return res.status(401).json({ message: 'Non autorisé, veuillez vous connecter en tant que restaurant.' });
        }
    
        const restaurantId = req.session.restaurant.id; // Obtenez l'ID du restaurant à partir de la session
        ...
        db.query(sql, [meal_name, meal_description, meal_price, meal_img, restaurantId], (err, result) => {
            if (err) {
